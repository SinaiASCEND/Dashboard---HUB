...
## How it is organised
- `index.html` — the Hub. The front page (Student and Faculty dashboard tiles, the access-code button, the two "Meet the…" links) is unchanged since it has been widely distributed. Behind the code there are seven codes: ASCEND (OCA leadership), EEC, PCCS, CCS, CIS, CCM (Clinical Competency Mentors), RPS (student performance only). Codes are stored as SHA-256 hashes of the upper-cased code in `TIERS`; cards are defined in `DASHBOARDS`.
